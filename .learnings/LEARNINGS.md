...
37. **重装 app 会静默作废「屏幕录制」授权，而系统设置里的勾看上去还勾着**：按 #36 的正确姿势重装（quit → `rm -rf` → `cp -R` → `codesign --verify` → `open`）之后，⌥A 照常工作但 ⌥S 一按就报「需要屏幕与系统音频录制权限」。两种权限的 TCC 记录粒度不同：**辅助功能**认代码签名身份（bundle id + 证书），同一份证书重签就活得下来（`bundle.sh` 用证书哈希而非名字签名正是为此）；**屏幕录制**额外绑定二进制的 cdhash，换了 build 记录即失效，但系统设置面板仍显示为已勾选——UI 与实际授权状态不一致，靠肉眼永远查不出来。修法：`tccutil reset ScreenCapture com.xvz.thorn` 清掉陈旧记录，重启 app 后重新授权；注意屏幕录制授权**必须重启进程才生效**（辅助功能是即时的，别用同一套直觉）。推论：每次给用户装新版本，都要顺手确认 ⌥S 还能用，别只测 ⌥A。

## 2026-08-01 破折号插入语的错切排查

38. **用户报"解析错了"，先证伪输入，再怀疑代码**："…admitting whatever Futurist poetry may be—it is at any rate…" 整树跑歪（`may/aux → admitting`、`whatever/det → poetry`），第一反应是 chunk 规则出错，实际是 spaCy 自己就这么给的：句中本该有一对破折号，前半那个在到达 Thorn 之前就没了（源文本录错或 OCR 吃掉了带空格的 ` — `），剩下一个孤立破折号让整句在语法上真的有歧义。判据是把同一句按「无破折号 / 补回破折号 / 换成逗号」三种写法各跑一遍依存树对比——补回去结构立刻正确，就说明代码没病、输入有病。同时反向验证 Thorn 两层规范化（Swift 侧 `normalizedInput`、sidecar 侧 `_surface_parse_text` / `_DASH_RUN`）都不删破折号，破折号是进门前就丢的。这类"输入本身歧义"绝不能靠句子指纹式启发式去救（见 #22：过拟合层删掉比打补丁强）。

39. **破折号紧跟系动词时，情态动词会被并进主语块**（已记录，暂不修）：把上面那句的破折号补全后整树正确，但切块给出 `[主语] Futurist poetry may` / `[谓语] be—`——`may` 跑到了主语里。触发条件很窄：谓语后面紧跟破折号。影响是一张卡片的边界偏一个词，不影响树结构和其余标注，故当时选择只记不修，避免和同一轮里另一个未提交的试行功能（语法笔记）搅在一起、把回滚边界弄脏。修的时候记得先按 #18 用真句子跑活管线复现，别只靠单测。

## 2026-08-01 短语动词与同位语从句
...
45. **先量再改：解析本身不是瓶颈，进程生命周期才是**：以为该优化解析速度，实测单句 30–45 ms（20 词 35 ms，25 词 43 ms），warm 与 first 几乎没差——没有任何东西值得优化。真正的开销全在冷启动：`import server`（torch+transformers+benepar）warm 1.0 s、页缓存被逐出后 **15.2 s**，`server.load()` 对应 1.9 s / 3.7 s，整体 3.2 s vs **19 s**。两个结论：(a) 原来 120 s 的 idle-exit 意味着"读完一段再查下一句"必然吃一次冷启动，改成 600 s；(b) `ensureHealthy` 的 30 s 轮询预算对 19 s 冷启动只有 1.5 倍余量，而页缓存被逐出恰恰发生在内存吃紧、一切都慢的时候——那时它会把一个只是还在加载的 sidecar 报成"引擎暂不可用"。放宽到 60 s，快的时候一分钱不花。

46. **同一个根因会在不同层级各犯一次：没人认领的 token 落进 leftover pass**：build_chunks 把每个 token 指派给某个 chunk root，指派不到的由 leftover pass 粘到最近的邻居——它本来只该收留零散的限定词和标点。两个真实缺陷都是它在收留**整个成分**：(a) 外置关系从句 "sent a spy to his house, who heard …"，关系从句挂在 `spy` 上但被 `to his house,` 隔开，名词自己的 Benepar NP 够不到它，11 个 token 被粘进 `[介词短语] to his house`；(b) 动词省略 "as perhaps Kelsey will after her resignation …"，两个 `after` 挂的是主句的 `discovered`，从句里的 `will` 没有子节点能认领，18 个 token 被粘进 `[谓语] will`。修法相同：在 leftover pass **之前**，把「dep 属于从句/介词、且自己一个 token 都没被认领」的中心词提升为本层的 chunk root。判据也相同——粘出来的卡片越长越像正确答案，因为它读起来仍然通顺。
...
52. **`isMovableByWindowBackground` 问的不是窗口，是鼠标落到的那个视图**：面板突然拖不动。这个属性本身只是开关，真正的决定权在 `hitTest` 命中的那个 view 的 `mouseDownCanMoveWindow`——而本面板每一个点都被 `NSHostingView` 盖住，于是"愿不愿意让窗口跟着走"一直是由 SwiftUI 塞在 `.background(.regularMaterial)` 底下那层材质视图替我们回答的。一个用户天天在用的交互，实现却挂在 SwiftUI 的私有视图布局上，它哪天换个摆法就无声失效，而且 git 里看不出任何改动。修法：自己放一层 `NSView`，压在材质**之上**、所有卡片**之下**，在 `mouseDown` 里调 `window.performDrag(with:)`。注意不是把 `mouseDownCanMoveWindow` 覆写成 `true`——那一版实测仍然拖不动，因为它还是走 AppKit 自己启动拖动的那条路，对"无边框 + 非激活面板 + 后台 App"这种窗口形态不成立；`performDrag` 直接拿已经到手的 mouse-down 跑拖动循环，没有这些前置条件。

53. **一个交互坏了，先修它，不要拿新控件把它替换掉**：第一版的判断是"这个功能从来没生效过"（git 里确实只有一行 `isMovableByWindowBackground = true`），于是加了一个专门的拖动抓手当作解法。用户一句"我以前拖任何地方都能拖"就否掉了整个方向——**用户的一手使用记忆是证据，代码里读不出来的历史不等于没发生过**。而且这个替换本身也是错的：它把"随手一拖"换成"先找到那个 20 pt 的图标"，用退化的交互去绕开一个还没定位的 bug，同时还挤宽了正文右侧留白。判据：当修法需要改变用户已有的操作方式时，先确认根因已经找到，否则改的是症状。

54. **两种失败长得一样，先加日志把它们分开，再决定修法**："拖不动"有两个完全不同的根因——鼠标压根没落到拖动层（命中测试问题，要改视图层级），和落到了但拖动没启动（API 选择问题，要换 `performDrag`）。前两版各修了其中一种，都是猜的，各烧掉一轮 build + 安装 + 人工验证。第三版在 `mouseDown` 里加了一行日志才把这两条路分开：有日志无位移 = 后者，无日志 = 前者。GUI 交互没法自动化验证（#51），所以每一轮人工验证都很贵，值得先花一行日志把候选根因砍掉一半。
